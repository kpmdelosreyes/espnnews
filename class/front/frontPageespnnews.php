...
class frontPageespnnews extends Controller_Front
{
	protected function run($aArgs)
	{
		...
		foreach($_aOrder as $key => $val) {
			$sClass = $key == 0 ? "on" : "off";
			$sText = $val == "College Basketball" ? "College Bask" : $val;
		...
		$sHtml .='<div id="pg_espn">
				...
				$sHtml .='<li><a href="#" onclick="espnnews.getNews(\''.$key['label']['tab1'].'\')" id="pg_espn_tab_'.$key['label']['tab1'].'" class="'.$key['class'].'"><span>'.$key['text']['tab1'].'</span></a></li>
						  <li><a href="#" onclick="espnnews.getNews(\''.$key['label']['tab2'].'\')" id="pg_espn_tab_'.$key['label']['tab2'].'" class="'.$key['class'].'"><span>'.$key['text']['tab2'].'</span></a></li>
						  <li><a href="#" onclick="espnnews.getNews(\''.$key['label']['tab3'].'\')" id="pg_espn_tab_'.$key['label']['tab3'].'" class="'.$key['class'].'"><span>'.$key['text']['tab3'].'</span></a></li>';
				}
				$sHtml .='</ul><div class="pg_espn_content_wrap">
							...
							$sHtml .='<li>
									...
									<img src="/_sdk/img/espnnews/pg_tree_p.gif" alt="Plus Sign" onclick="espnnews.plus(this);"/>
									<img src="/_sdk/img/espnnews/pg_tree_m.gif" alt="Minus Sign" style="display:none" onclick="espnnews.minus(this);" />
									</span>
